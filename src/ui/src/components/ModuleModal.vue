          ...
          <v-expansion-panels>
            <v-expansion-panel v-for="action in modal_text.actions" :key="action.name">
              ...
              <v-expansion-panel-text>
                ...
                <v-data-table :headers="arg_headers" :items="action.args" hover items-per-page="-1"
                  ...
                  <template v-slot:item="{ item }: { item: any }">
                    <tr>
                      ...
                      <td v-if="item.type == 'Location'"><v-select @update:menu="set_text(action)" v-model="item.value" :items="Object.keys(wc_state.locations)"></v-select></td>
                      <td v-else><v-text-field @update:focused="set_text(action)"height="20px" v-model="item.value" dense>
                        </v-text-field></td>
                    </tr>
                  </template>
                  <template #bottom></template>
                </v-data-table>
                <v-data-table v-if="action.files.length > 0" :headers="file_headers" :items="action.files" hover items-per-page="-1"
                  no-data-text="No Files" density="compact">
                  <template v-slot:item="{ item }: { item: any }">
                    <tr>
                      <td>{{ item.name }}</td>
                      <td>{{ item.required }}</td>
                ...
                <v-btn @click="copy= !copy; set_text(action)"><p v-if="(copy==false)">Show Copyable Workflow Step</p> <p v-else>Hide copyable workflow step</p></v-btn>
                <div v-if="copy">
...
async function send_wf(action:any) {
  ...
  var test: any = await info.json()
  console.log(test)
  console.log(test["experiment_id"])
  formData.append("experiment_id", test["experiment_id"])
  action.files.forEach(function(file: any) {
  if(file.value) {
