...
class TestWEI_Workflows(TestWEI_Base):
    ...
    def test_create_value_datapoint(self):
        ...
        self.experiment.create_datapoint(datapoint)

        assert self.experiment.get_datapoint_value(datapoint.id)["value"] == 42
        uploaded_datapoint = self.experiment.get_datapoint_info(datapoint.id)
        ...
        assert (
            ...
            == self.experiment.experiment_info.experiment_id
        )
        assert (
            uploaded_datapoint.campaign_id
            == self.experiment.experiment_info.campaign_id
        )
    ...
    def test_create_local_file_datapoint(self):
        ...
        self.experiment.create_datapoint(datapoint)
        assert isinstance(self.experiment.get_datapoint_value(datapoint.id), bytes)
        uploaded_datapoint = self.experiment.get_datapoint_info(datapoint.id)
        ...
        self.experiment.save_datapoint_value(datapoint.id, "download.py")
